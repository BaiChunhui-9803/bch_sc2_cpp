...
namespace ofec {
	namespace nd_sort {
		namespace filter_sort {
			...
			bool greater(const std::vector<T>* a, const std::vector<T>* b, const int obj_idx, const std::vector<OptMode>& opt_mode) {
				...
				for (size_t i = 0; i < obj_num; ++i) {
					...
					if (opt_mode[idx] == OptMode::kMinimize) {
			...
			template<typename T>
			void mergeSortColRecur(const std::vector<std::vector<T>*>& data, const int obj_idx, std::vector<int>& B, std::vector<int>& A, int l, int r, const std::vector<OptMode>& opt_mode) {
				if (r - l < 2)
					return;
				int m = (r + l) / 2;
			...
			template<typename T>
			void mergeSortCol(const std::vector<std::vector<T>*>& data, std::vector<int>& index, const int obj_idx, const std::vector<OptMode>& opt_mode) {
			...
			template<typename T>
			void parallelMergeSort(const std::vector<std::vector<T>*>& data, std::vector<std::vector<int>>& SeqByObj, std::vector<int> ObjIdxs, const std::vector<OptMode>& opt_mode) {
		...
		template<typename T>
		int filterSort(const std::vector<std::vector<T>*>& data, std::vector<int>& rank, const std::vector<OptMode>& opt_mode) {
			...
			delete[] in_cur_front;
			return cur_rank_number - 1;
		}

		template<typename T>
		int filterSortP(const std::vector<std::vector<T>*>& data, std::vector<int>& rank, const std::vector<OptMode>& opt_mode, size_t num_task = -1) {
			if (num_task == -1)
