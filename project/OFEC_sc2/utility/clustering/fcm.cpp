...
void FCM::init(std::vector<std::vector<double>> &Data_points, int id_rnd) {
	...
	for (int i = 0; i < m_num_data_points; ++i) {
		...
		for (int j = 1; j < m_num_clusters; ++j) {
			rval = OFEC::GET_RND(id_rnd).uniform.nextNonStd<size_t>(0, r + 1);
			//rval = rand() % (r + 1);
			r -= rval;
...
void FCM::clustering(std::vector<std::vector<double>> &Data_points, FCM::Result &result, int id_rnd)
{
	double max_diff;
	init(Data_points, id_rnd);
	do {
