...
namespace ofec {
	...
	public:
		FDC(const Population<TInd> &pop, int id_pro) :
			m_fitness(pop.size()),
			m_distance(pop.size()),
			...
			m_obj_dist_calulated(false)
		{
			for (int pop_idx(0); pop_idx < pop.size(); ++pop_idx) {
				...
				for (int other_idx(0); other_idx < pop_idx; ++other_idx) {
					m_distance[other_idx][pop_idx] = pop[pop_idx].variableDistance(pop[other_idx], id_pro);
					m_distance[pop_idx][other_idx] = pop[pop_idx].variableDistance(pop[other_idx], id_pro);
				}
			}
		...
		FDC(const std::vector<std::unique_ptr<TInd>> &pop, int id_pro) :
			m_fitness(pop.size()),
			m_distance(pop.size()),
			m_fitness_distance(pop.size()),
			m_belong(pop.size(), -1),
		...
		FDC(const std::vector<TInd> &pop, int id_pro) :
			m_fitness(pop.size()),
			m_distance(pop.size()),
			m_fitness_distance(pop.size()),
			m_belong(pop.size(), -1),
