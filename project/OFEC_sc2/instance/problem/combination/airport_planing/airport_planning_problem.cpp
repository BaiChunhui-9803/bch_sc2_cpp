...
void airport_planning_problem::algSolutionRandom2Mem(int id_rnd) const {
	solution_type s;
...
void airport_planning_problem::initSolutionRandomRandMem(SolBase& s, int id_rnd) const {
	auto& x = dynamic_cast<solution_type&>(s).variable().vect();
	x.resize(m_numFlight);
	for (int fId(0); fId < m_numFlight; ++fId) {
		auto& memberSeq(x[fId]);
		memberSeq.resize(
			GET_RND(id_rnd).uniform.nextNonStd<int>(m_flights[fId].m_CompTotNum, m_MaxDH));
		int captianNum(m_C_FO.first.size()), fONum(m_C_FO.second.size());
		for (int idx(0); idx < m_flights[fId].m_CompNum.first; ++idx) {
			int captianIdx =GET_RND(id_rnd).uniform.nextNonStd<int>(0, captianNum);
			int add(0);
		...
		for (int idx(m_flights[fId].m_CompNum.first); idx < m_flights[fId].m_CompTotNum; ++idx) {
			int captianIdx = GET_RND(id_rnd).uniform.nextNonStd<int>(0, fONum);
			int add(0);
		...
		for (int idx(m_flights[fId].m_CompTotNum); idx < memberSeq.size(); ++idx) {
			int captianIdx = GET_RND(id_rnd).uniform.nextNonStd<int>(0, fONum + captianNum);
...
void airport_planning_problem::initSolutionRandomNetwork(SolBase& s, int id_rnd) const {
	SolState cur_sols;
	...
	GET_RND(id_rnd).uniform.shuffle(memberSeq.begin(), memberSeq.end());
	int memberSeqId(0);
	int memberId = 0;
	...
	while (cur_sols.m_unfeasibleFlightNum && cur_sols.m_feasibleMemberNum&&totalLoop--) {
		 ...
		 // construct a solutions
		{
			...
			while (true) {
				...
				if (!feasible.empty()) {
					next_step= feasible[GET_RND(id_rnd).uniform.nextNonStd<int>(0,feasible.size())];
				}
				if (next_step == -1)
				{
					break;
...
bool OFEC::APP::airport_planning_problem::dfsSolutions(SolState& cur, std::vector<bool>& feasibleNode, int id_rnd) const
{
	return false;
}
...
void airport_planning_problem::initSolution(SolBase& s, int id_rnd) const
{
     
    //std::vector<int> cur
}

void OFEC::APP::airport_planning_problem::printSol(ostream& out, SolBase& s) const
{
	auto& x = dynamic_cast<solution_type&>(s).variable().vect();
...
void OFEC::APP::airport_planning_problem::printSolReal(ostream& out, SolBase& s) const
{
...
void OFEC::APP::airport_planning_problem::printSolObj(ostream& out, SolBase& s) const
{
...
void airport_planning_problem::calObj(SolBase& s)const {
...
bool airport_planning_problem::better2Obj(SolBase& s1, SolBase& s2) const {
...
void OFEC::APP::airport_planning_problem::transferSolutions(AAP_struct& cur, const VarBase& s1)const
{
...
void OFEC::APP::airport_planning_problem::transferSolutions(SolBase& s, SolState& cur_sol)const
{
	auto& x = dynamic_cast<solution_type&>(s).variable().vect();
	x.resize(m_numFlight);
...
void OFEC::APP::airport_planning_problem::evaluate_(SolBase& s, bool effective)
{
    
}

bool airport_planning_problem::same(const SolBase& s1, const SolBase& s2) const
{
	return false;
}

Real airport_planning_problem::variableDistance(const SolBase& s1, const SolBase& s2) const
{
	return Real();
}

Real airport_planning_problem::variableDistance(const VarBase& s1, const VarBase& s2) const
{
	return Real();
}
