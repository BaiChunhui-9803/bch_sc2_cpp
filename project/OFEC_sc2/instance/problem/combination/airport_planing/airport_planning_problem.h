//Register ComOP_APP "ComOP_APP" APP,ComOP,DOP
...
#define GET_APP(id_pro) dynamic_cast<APP::airport_planning_problem&>(InstanceManager::ms_instance_manager->getProblem(id_pro))
	...
	namespace APP {
		...
		protected:
			...
			const std::string m_filepath = static_cast<std::string>(OFEC::g_working_dir) + "/instance/problem/combination/airport_planing/data/";
			...
			void transferSolutions(AAP_struct& cur, const VarBase& s1)const;
			// 某些飞机不满足飞行条件
			void transferSolutions(SolBase& s,  SolState& s1)const;
			...
			*/
			...
			bool dfsSolutions(SolState& cur, std::vector<bool>& feasibleNode,int id_rnd)const;
		public:
			using variable_type = VarVec<std::vector<int>>;
			using solution_type = Solution<variable_type>;
		
		public:

			//initSolutions with two members
			void initSolutionRandomNetwork(SolBase& s, int id_rnd) const;

			void algSolutionRandom2Mem(int id_rnd) const;
			void initSolutionRandom2Mem(SolBase& s, int id_rnd) const ;
			void initSolutionRandomRandMem(SolBase& s, int id_rnd) const;
			void printSol(std::ostream& out, SolBase& s)const;
			void printSolReal(std::ostream& out, SolBase& s)const;
			void printSolObj(std::ostream& out, SolBase& s)const;
			void calObj(SolBase& s)const;
			bool better2Obj(SolBase& s1, SolBase& s2) const;
			// 通过 problem 继承
