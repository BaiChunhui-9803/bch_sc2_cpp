...
void OFEC::SP::wall_info::set_position_style(int id_rnd,Real* center, wall_type* type, Real max_radius, Real small_loop_radius, Real large_loop_inner)
{
	if (center != nullptr)
		m_center = *center;
	if (type != nullptr)
	...
	case wall_type::small_loop: {
		m_inner_radius = 0;
		m_outer_radius = GET_RND(id_rnd).uniform.nextNonStd<Real>(0, small_loop_radius);
		break;
	}
	case wall_type::large_loop: {
		
		m_outer_radius = GET_RND(id_rnd).uniform.nextNonStd<Real>(large_loop_inner, max_radius);
		m_inner_radius = m_outer_radius - large_loop_inner;
		break;
	}

	case wall_type::needle: {
		m_outer_radius = GET_RND(id_rnd).uniform.nextNonStd<Real>(large_loop_inner + 0.1, max_radius);
		Real loop_r = GET_RND(id_rnd).uniform.nextNonStd<Real>(0, m_outer_radius);
		m_inner_radius = m_outer_radius - loop_r;

		m_from_theta = GET_RND(id_rnd).uniform.nextNonStd<Real>(0, 2 * OFEC_PI);
		m_to_theta = GET_RND(id_rnd).uniform.nextNonStd<Real>(0, 2 * OFEC_PI);
	...
	default: {
		m_inner_radius = 0;
		m_outer_radius = GET_RND(id_rnd).uniform.nextNonStd<Real>(small_loop_radius, max_radius);
		break;
	}
...
int OFEC::SP::wall_info::cal_numPos(int id_rnd,wall_type type, int num_small_points,
	...
	{
	case wall_type::small_loop: {
		numPos = GET_RND(id_rnd).uniform.nextNonStd<int>(5, num_small_points);
	...
	case wall_type::large_loop: {
		numPos = GET_RND(id_rnd).uniform.nextNonStd<int>(5, num_large_points);
		break;
	}

	case wall_type::needle: {
		numPos = GET_RND(id_rnd).uniform.nextNonStd<int>(5, num_needle_points);
		break;
	}
	default: {
		numPos = GET_RND(id_rnd).uniform.nextNonStd<int>(5, num_circle_points);
		break;
	}
	}
	return numPos;
...
void OFEC::SP::wall_info::set_numPos(int id_rnd,int num_small_points, int num_large_points, int num_circle_points, int num_needle_points)
{

	m_numPos = cal_numPos(id_rnd,m_type, num_small_points, num_large_points, num_circle_points, num_needle_points);
}



void  OFEC::SP::map_info::initialize_wall_type(int id_rnd) {
	//m_points.clear();
	...
	GET_RND(id_rnd).uniform.shuffle(random_needle.begin(), random_needle.end());
	...
	GET_RND(id_rnd).uniform.shuffle(modility.begin(), modility.end());
	std::vector<wall_type> type;

	int type_idx(GET_RND(id_rnd).uniform.nextNonStd<int>(0, random_needle.size()));
	...
	for (int dim_idx(0); dim_idx < variable_size; ++dim_idx) {
		Real wall_center(0);
		if (dim_idx) {
			wall_center = m_wall_types[dim_idx - 1].m_center + GET_RND(id_rnd).uniform.nextNonStd<Real>(m_parameter.m_width_range.first, m_parameter.m_width_range.second);
		}
		m_wall_types[dim_idx].set_position_style(id_rnd,&wall_center, &type[dim_idx], m_parameter.m_max_radius, m_parameter.m_small_loop_radius, m_parameter.m_large_loop_inner);
		m_wall_types[dim_idx].set_numPos(id_rnd,m_parameter.m_num_small_points, m_parameter.m_num_large_points, m_parameter.m_num_circle_points, m_parameter.m_num_needle_points);
		m_wall_types[dim_idx].m_bag_value_seed = GET_RND(id_rnd).uniform.next();
		m_wall_types[dim_idx].m_center_id = GET_RND(id_rnd).uniform.nextNonStd<int>(0, m_wall_types[dim_idx].m_numPos);
...
void  OFEC::SP::map_info::initialize(int id_rnd) {
	...
	initialize_wall_type(id_rnd);
	m_discount.resize(m_parameter.m_station_type_num);
	for (auto& it: m_discount) {
		it.first = GET_RND(id_rnd).uniform.nextNonStd<Real>(m_parameter.m_ratio_level.first, m_parameter.m_ratio_level.second);
		it.second= GET_RND(id_rnd).uniform.nextNonStd<Real>(m_parameter.m_discount.first, m_parameter.m_discount.second);
	}
	...
	for (int wall_id(0); wall_id < m_wall_types.size(); ++wall_id) {
		...
		for (int pos_id(0); pos_id < m_wall_types[wall_id].m_numPos; ++pos_id) {
			...
			cur_station.m_cor_id = GET_RND(id_rnd).uniform.nextNonStd<int>(0, m_parameter.m_station_type_num);
			
			cur_station.m_static_flag =  GET_RND(id_rnd).uniform.next() > m_parameter.m_moving_station_ratio;
...
void OFEC::SP::SelectionProblem::updateParameters()
{
}

bool OFEC::SP::SelectionProblem::same(const SolBase& s1, const SolBase& s2) const
{
	auto& x1 = dynamic_cast<const Solution<VarVec<int>>&>(s1).variable();
	auto& x2 = dynamic_cast<const Solution<VarVec<int>>&>(s2).variable();
	return x1.vect() == x2.vect();
}

Real OFEC::SP::SelectionProblem::variableDistance(const SolBase& s1, const SolBase& s2) const
{
	auto& x1 = dynamic_cast<const Solution<VarVec<int>>&>(s1).variable();
	auto& x2 = dynamic_cast<const Solution<VarVec<int>>&>(s2).variable();
	...
	Real dis(0);
	Real maxDis(m_num_vars * m_num_vars);

	for (int i(0); i < m_num_vars; ++i) {
		if (x1[i] == x2[i]) {
			dis += 1;
			if (i + 1 < m_num_vars && x1[i + 1] == x2[i + 1]) {
				dis += m_num_vars;
			}
		}
	}
...
Real OFEC::SP::SelectionProblem::variableDistance(const VarBase& s1, const VarBase& s2) const
{
...
void OFEC::SP::SelectionProblem::evaluate_(SolBase& s, bool effective) {
...
//		x[i] = GET_RND(id_rnd).uniform.nextNonStd<int>(0, m_map.m_wall_types[i].m_numPos);
//	}
//	
//}


void OFEC::SP::SelectionProblem::initSolution(SolBase& s, int id_rnd) const  {
	...
	for (int i(0); i < x.size(); ++i) {
		x[i] =GET_RND(id_rnd).uniform.nextNonStd<int>(0, m_map.m_wall_types[i].m_numPos);
		//GET_RND(id_rnd).
	//	x[i] = GET_RND(id_rnd).uniform.nextNonStandard<int>(0, m_map.m_wall_types[i].m_numPos);
	}
...
void OFEC::SP::SelectionProblem::initialize_()
{
	...
	*/
	addTag(ProTag::DOP);
	addTag(ProTag::NoisyOP);


	m_T = 0;
	m_map.initialize(m_id_rnd);
	m_num_vars = m_map.m_wall_types.size();

	m_num_objs = 1;
	m_num_cons = 0;
	//m_frequency = 1000;
	m_opt_mode.resize(m_num_objs);
	m_opt_mode.front()=OptMode::Minimize;
...
//void OFEC::SP::SelectionProblem::generate_HLS_samples(const SolBase& s, std::vector<std::unique_ptr<SolBase>>& samples, int id_rnd) {
...
//	}
//
//	//std::vector<int> dim_idxs(m_num_vars);
//	//for (int idx(0); idx < dim_idxs.size(); ++idx) {
//	//	dim_idxs[idx] = idx;
//	//}
//	//GET_RND(id_rnd).uniform.shuffle(dim_idxs.begin(), dim_idxs.end());
//
//	//int sample_num(static_cast<double>(samples.size()) / m_num_vars);
//	//int sample_left(samples.size() - sample_num * m_num_vars);
//	//int cur_sample(0), cur_dim(0);
//	//for (int dim_idx(0); dim_idx < m_num_vars; ++dim_idx) {
...
//	//			pos_idxs[idx] = idx;
//	//		}
//	//		GET_RND(id_rnd).uniform.shuffle(pos_idxs.begin(), pos_idxs.end());
//
...
//
//	//		for (int pos_idx(0); pos_idx < m_num_vars; ++pos_idx) {
//	//			cur_sample = sample_num;
//	//			if (dim_idx < sample_left) ++cur_sample;
//	//			if (cur_sample)
//	//		}
...
Real OFEC::SP::SelectionProblem::evaluate_mean_value(SolBase& s)
{
	auto& x = dynamic_cast<solution_type&>(s).variable();
	bool feasible_flag(false);
	return m_map.get_mean_value(m_T, x.vect(), feasible_flag);
}
void OFEC::SP::SelectionProblem::generate_samples_numbers(std::vector<int>& idx_samples, int sample_num, int id_rnd) {
...
void OFEC::SP::SelectionProblem::showInfomations(int id_alg)
{
	if (!GET_ALG(id_alg).candidates().empty()) {
		printfSolution(id_alg, *GET_ALG(id_alg).candidates().front());
	}
}
void OFEC::SP::SelectionProblem::printfSolution(int id_alg,const SolBase& sol)
{
	auto& x = dynamic_cast<const solution_type&>(sol).variable();
	std::cout << "effective_eval \t" << GET_ALG(id_alg).numEffectiveEvals()
...
std::vector<int> OFEC::SP::SelectionProblem::generate_samples_numbers(int size, int sample_num,int id_rnd)
{
	std::vector<int> idxs(size);
	for (int idx(0); idx < idxs.size(); ++idx) {
		idxs[idx] = idx;
...
void OFEC::SP::SelectionProblem::generate_level_samples(std::vector<solution_type>& samples,int id_rnd)
{
	samples.clear();
	int max_nodes(0);
	...
	for (int dim(0); dim < m_map.m_wall_types.size(); ++dim) {
		...
		GET_RND(id_rnd).uniform.shuffle(solus[dim].begin(), solus[dim].end());
...
Real OFEC::SP::SelectionProblem::evaluate_effective_value(SolBase& s) {
