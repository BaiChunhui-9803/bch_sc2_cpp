...
namespace ofec {
	void WFG::initialize_() {
		...
		int k_factor = v.count("k factor") > 0 ? std::get<int>(v.at("k factor")) : 1;
		m_k = k_factor * (m_num_objs - 1);

		resizeVariable(std::get<int>(v.at("number of variables")));
		if (m_name == "MOP_WFG2" || m_name == "MOP_WFG3") {
			int l_factor = v.count("l factor") > 0 ? std::get<int>(v.at("l factor")) : 2;
			if (m_num_vars != m_k * 2 * l_factor)
		...
		m_D = 1;
		if (m_name == "MOP_WFG3")
			m_A.assign(m_num_objs - 1, 1);
		else
			m_A.assign(m_num_objs - 1, 0);
		m_A[0] = 1;
	...
	void WFG::evaluateObjective(Real *x, std::vector<Real> &obj) {
		...
		for (int i = 0; i < m_num_objs; i++) {	//calculate objective value
