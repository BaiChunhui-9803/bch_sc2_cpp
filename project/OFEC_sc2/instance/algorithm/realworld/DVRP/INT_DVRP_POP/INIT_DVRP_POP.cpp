#include "INIT_DVRP_POP.h"
#include <thread>
namespace ofec {
	void INIT_DVRP_POP::ahc_tsp_init_pop(std::vector<DVRP_ind> &pop)
	{
		...
		//	AHC_TSP ahc(weight, DVRP_CAST->get_current_cus_order());
		...
		//		x.m_cus_order.back().push_back(DVRP_CAST->get_depot());
		...
		//	}
		//	DVRP_CAST->construct_path(*pop[i]);
			
		//}
		
	}
	void INIT_DVRP_POP::ahc_tsp_init_pop_(DVRP_ind& ind, int i)
	{
		std::cout << "initialize pop " << std::to_string(i) << std::endl;
		auto &x = ind->variable();
		//随机生成权重
	...
	void INIT_DVRP_POP::randomly_init_pop(std::vector<DVRP_ind>& pop)
	{
		for (int i = 0; i < pop.size(); ++i) {
			std::cout << "initialize pop " << std::to_string(i) << std::endl;
			DVRP_CAST->initialize_solution(*pop[i]);
		}
	}
	void INIT_DVRP_POP::ahc_initialize_pop(std::vector<DVRP_ind>& pop)
	{
		for (int i = 0; i < pop.size(); ++i) {
			auto &x = pop[i]->variable();
			Real weight = global::ms_global->m_uniform[caller::Algorithm]->next_non_standard<Real>(0, 1);
			...
			AHC_v2 ahc(weight, DVRP_CAST->get_current_cus_order());
			ahc.clustering(result);

			std::vector<std::vector<size_t>> m_cus_order;
			m_cus_order = result.member;

			calcu_cus_order(m_cus_order);
	...
	void INIT_DVRP_POP::calcu_cus_order(std::vector<std::vector<size_t>>& m_cus_order)
	{
		for (int i = 0; i < m_cus_order.size(); ++i) {
			m_cus_order[i] = tsp(m_cus_order[i]);
		}
	}
	std::vector<size_t> INIT_DVRP_POP::tsp(std::vector<size_t>& member_index)
	{
		...
		while (!member_index.empty()) {
			...
			for (int i = 0; i < member_index.size(); ++i) {
				...
				get_cost(start, member_index[i], present_time);//此处耗时可能太长
				...
				Real diff_tw = 0.0;
				Real beginTime = DVRP_CAST->get_net().get_road_net()[member_index[i]]->datum_t.begin_time;
				Real endTime = DVRP_CAST->get_net().get_road_net()[member_index[i]]->datum_t.end_time;
			...
			//Time = present_time;
				...
				Time = std::get<2>(member_cost[index]) + DVRP_CAST->get_net().get_road_net()[member_index[index]]->server_time;
			}
			//Time = present_time + DVRP_CAST->get_net().get_road_net()[DVRP_CAST->get_current_cus_order()[member_index[index]]]->server_time;
			start = next;
			tsp.push_back(next);
	...
	void INIT_DVRP_POP::get_cost(size_t s, size_t e, Real &present_time)
	{
		//present_time = present_time + DVRP_CAST->get_cost_time(present_time,s,e);
		//present_time = present_time + DVRP_CAST->get_cost_time()[s][e];
		present_time = present_time + DVRP_CAST->get_cost_time(present_time, s, e);
	}
