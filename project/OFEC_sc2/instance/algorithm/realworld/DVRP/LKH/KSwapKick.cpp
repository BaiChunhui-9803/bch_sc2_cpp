 ...
 */
namespace LKH {
	static LKHAlg::Node *RandomNode(LKHAlg *Alg);
	static int compare(const void *Na, const void *Nb);

	void LKHAlg::KSwapKick(int K)
	{
		Node **s, *N;
		int Count, i;
	...
	static LKHAlg::Node *RandomNode(LKHAlg *Alg)
	{
		...
		Count = 0;
		while ((N->V || (Fixed(N, N->Suc) || Alg->IsCommonEdge(N, N->Suc))) && Count < Alg->Dimension) {
			N = N->Suc;
			Count++;
		}
	...
	static int compare(const void *Na, const void *Nb)
	{
		return (*(LKHAlg::Node **) Na)->Rank - (*(LKHAlg::Node **) Nb)->Rank;
	}
}