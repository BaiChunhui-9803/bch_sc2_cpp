 ...
 */
namespace LKH {
	GainType LKHAlg::MTSP_InitialTour()
	{
		...
		for (i = 1; i <= Dim; i++) {
			...
			if (N->Special) {
				...
				N->Cost = ((this->*C)(N, Route) - N->Pi - Route->Pi) / Precision;
				Route->Cost = 2 * N->Cost;
				Route->V++;
				Cost += Route->Cost;
			}
		...
		for (i = 1; i <= Dim; i++) {
			...
			if (N == Depot || N->Special)
				continue;
			N->Rank = ((this->*C)(N, Depot) - N->Pi - Depot->Pi) / Precision;
		...
		while ((N = HeapDeleteMin(this))) {
			...
			for (i = 0; i < Salesmen; i++) {
				...
				if (Route->V == MTSPMaxSize)
					continue;
				Last = Route->Pred;
				d = ((this->*C)(Last, N) - Last->Pi - N->Pi) / Precision +
					N->Cost - Last->Cost;
			...
			Last = Winner->Pred;
			if (MTSPObjective == MINMAX || MTSPObjective == MINMAX_SIZE)
				Winner->Cost += ((this->*C)(Last, N) - Last->Pi - N->Pi) / Precision +
				N->Cost - Last->Cost;
			else
				Winner->Cost += Min;
			Cost += Winner->Cost;
