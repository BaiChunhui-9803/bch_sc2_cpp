...
#define Level V
	...
	static int BetaValue(LKHAlg::Node * From, LKHAlg::Node * To, LKHAlg *Alg);
	static LKHAlg::Candidate *FindCandidate(LKHAlg::Node * From, LKHAlg::Node * To);
#undef max
	static int max(const int a, const int b);

	void LKHAlg::OrderCandidateSet(int MaxCandidates, GainType MaxAlpha, int Symmetric)
	{
		Node *From, *To, *N;
		Candidate *NFrom, *NN;
		int Alpha, Beta;
				...
				for (i = 0; i < From->OriginalCandidates; i++) {
					...
					for (j = 0; j < N->OriginalCandidates; j++) {
						...
						if (FixedOrCommon(From, To))
							...
							if (Beta == INT_MIN)
								continue;
							Alpha = max((this->*D)(From, To) - Beta, 0);
						}
						if (Alpha <= MaxAlpha)
							AddCandidate(From, To, (this->*D)(From, To), Alpha);
					}
				}
				From->AlphaComputed = 1;
			} while ((From = From->Suc) != FirstNode);
	...
	static int BetaValue(LKHAlg::Node * From, LKHAlg::Node * To, LKHAlg *Alg)
	{
		...
		if (From == To->Dad)
			return To->Cost;
		if (From == Alg->FirstNode || To == Alg->FirstNode)
			return Alg->FirstNode->NextCost;
