...
#define PRANDMAX INT_MAX
	...
	static void WindowSize(double XMin, double XMax, double YMin, double YMax,
		double ZMin, double ZMax, int start, int end, LKHAlg *Alg);
	static void MakeSubproblem(double XMin, double XMax, double YMin,
		double YMax, double ZMin, double ZMax,
		int Subproblem, int start, int end, LKHAlg *Alg);
	static thread_local int Size;
	static thread_local LKHAlg::Node **KDTree;
	...
	void LKHAlg::SolveRoheSubproblems()
	{
		...
		while (Remaining > SubproblemSize) {
			...
			MakeSubproblem(N->X - CMid * DX, N->X + CMid * DX,
				N->Y - CMid * DY, N->Y + CMid * DY,
				N->Z - CMid * DZ, N->Z + CMid * DZ,
				++Subproblems, 0, Dimension - 1, this);
			Remaining -= Size;
		}
		if (Remaining > 3) {
			Subproblems++;
		...
		if (end - start + 1 <= Alg->SubproblemSize) {
			...
			if (Size <= Alg->SubproblemSize &&
				(N->Axis == 0 ? N->X >= XMin : N->Axis == 1 ? N->Y >= YMin :
					N->Z >= ZMin))
				WindowSize(XMin, XMax, YMin, YMax, ZMin, ZMax, start, mid - 1, Alg);
			if (Size <= Alg->SubproblemSize &&
				(N->Axis == 0 ? N->X <= XMax : N->Axis == 1 ? N->Y <= YMax :
					N->Z <= ZMax))
				WindowSize(XMin, XMax, YMin, YMax, ZMin, ZMax, mid + 1, end, Alg);
		}
	}
