...
#define Level BestPi    /* Search level */
	...
	static LKHAlg::Node *NearestNeighbor(LKHAlg::Node * From, LKHAlg *Alg);
	static LKHAlg::Node *NearestInList(LKHAlg::Node * From, LKHAlg::Node * First, LKHAlg *Alg);
	static int MayBeAddedToFragments(LKHAlg::Node * From, LKHAlg::Node * To, LKHAlg *Alg);
	static void AddEdgeToFragments(LKHAlg::Node * From, LKHAlg::Node * To, LKHAlg *Alg);
	...
	GainType LKHAlg::GreedyTour()
	{
		...
		for (;;) {
			if ((To = NearestNeighbor(From, this)) && FixedOrCommon(From, To))
				AddEdgeToFragments(From, To, this);
			else {
				if ((From->Nearest = To)) {
