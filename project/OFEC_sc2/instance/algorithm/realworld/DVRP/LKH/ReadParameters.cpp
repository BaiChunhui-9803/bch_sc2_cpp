 ...
 */
namespace LKH {
	static thread_local char Delimiters[] = "= \n\t\r\f\v\xef\xbb\xbf";
	static char *GetFileName(char *Line);
	static char *ReadYesOrNo(int *V);
#undef max
	static size_t max(size_t a, size_t b);

	void LKHAlg::ReadParameters()
	{
		char *Line = 0, *Keyword = 0, *Token = 0, *Name = 0;
		unsigned int i;
		...
		//Crossover.reset(new CrossoverFunction(ERXT));
		Crossover = ERXT;
		...
		/*if (!MaxPopulationSize.get())
		{
			...
			PopulationSize.reset(new int(0));
		}*/
		MaxSwaps = -1;
		MaxTrials = -1;
		MoorePartitioning = 0;
