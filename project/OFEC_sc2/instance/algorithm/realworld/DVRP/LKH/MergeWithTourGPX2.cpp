 ...
 */
namespace LKH {
	GainType LKHAlg::MergeWithTourGPX2()
	{
		int NewDimension = 0;
		...
		i = 0;
		do {
			Cost1 += (this->*C)(N, N->Suc) - N->Pi - N->Suc->Pi;
		...
		do {
			...
			red[i] = N->Rank = i;
			i++;
			ShrunkCost1 += (this->*C)(N, N->OldSuc) - N->Pi - N->OldSuc->Pi;
		} while ((N = N->OldSuc) != First);
		i = 0;
		do {
			blue[i++] = N->Rank;
			ShrunkCost2 += (this->*C)(N, N->Next) - N->Pi - N->Next->Pi;
		} while ((N = N->Next) != First);
		ShrunkCost1 /= Precision;
		ShrunkCost2 /= Precision;

		/* Perform GPX2 recombination */
		NewCost = gpx(red, blue, offspring, this);
		free(red);
		free(blue);
		...
		Cost1 = 0;
		do {
			Cost1 += (this->*C)(N, N->OldSuc) - N->Pi - N->OldSuc->Pi;
