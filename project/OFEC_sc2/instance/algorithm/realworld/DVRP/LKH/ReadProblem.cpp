...
void LKHAlg::ReadProblem()
{
    int i, j, K;
    char *Line, *Keyword;

	ostringstream FileName;
	FileName << static_cast<string>(OFEC::g_working_dir) + "/instance/algorithm/realworld/DVRP/LKH/temp file/"+ ProblemFileName;
...
static void Read_NAME(LKHAlg *Alg)
{
    if (!(Alg->Name = Copy(strtok(0, Delimiters))))
...
static void Read_DISPLAY_DATA_TYPE(LKHAlg *Alg)
{
    ...
    if (!(Alg->DisplayDataType = Copy(strtok(0, Delimiters))))
...
static void Read_EDGE_DATA_FORMAT(LKHAlg *Alg)
{
    ...
    if (!(Alg->EdgeDataFormat = Copy(strtok(0, Delimiters))))
...
static void Read_EDGE_WEIGHT_FORMAT(LKHAlg *Alg)
{
    ...
    if (!(Alg->EdgeWeightFormat = Copy(strtok(0, Delimiters))))
	...
	case LKH::FULL_MATRIX:
        for (i = 1; i <= Alg->Dim; i++) {
            ...
            for (j = 1; j <= Alg->Dim; j++) {
                ...
                W = round(Alg->Scale * w);
                if (W > INT_MAX / 2 / Alg->Precision)
                    W = INT_MAX / 2 / Alg->Precision;
...
static void Read_EDGE_WEIGHT_TYPE(LKHAlg *Alg)
{
    ...
    if (!(Alg->EdgeWeightType = Copy(strtok(0, Delimiters))))
...
static void Read_NODE_COORD_TYPE(LKHAlg *Alg)
{
    ...
    if (!(Alg->NodeCoordType = Copy(strtok(0, Delimiters))))
...
static void Read_TYPE(LKHAlg *Alg)
{
    ...
    if (!(Alg->Type = Copy(strtok(0, Delimiters))))
