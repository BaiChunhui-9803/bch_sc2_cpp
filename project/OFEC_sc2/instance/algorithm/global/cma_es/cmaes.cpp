...
/* --- Changes : ---
  ...
  03/03/29: restart via cmaes_resumeDistribution() implemented.
...
double *
cmaes_init_final(cmaes_t *t, int id_rnd)
/*
 * */
{
	int i, j, N;
	...
	if (!t->sp.flgsupplemented) {
		...
		if (!isNoneStr(t->sp.filename)) /* TODO: should this be done in readpara_SupplementDefaults? */
			cmaes_readpara_WriteToFile(&t->sp, (ofec::g_working_dir + "/result/actparcmaes.par").c_str());
	}
	...
	if (t->sp.typicalXcase) {
		...
		for (i = 0; i < N; ++i)
			t->rgxmean[i] += t->sigma * t->rgD[i] * GET_RND(id_rnd).normal.next();//cmaes_random_Gauss(&t->rand);
	}
	if (strcmp(t->sp.resumefile, "_no_") != 0)
		cmaes_resumeDistribution(t, t->sp.resumefile);
	...
	int id_rnd)
{
	cmaes_init_para(t, dimension, inxstart, inrgstddev, inseed,
		lambda, input_parameter_filename);
	return cmaes_init_final(t, id_rnd);
}
...
void
cmaes_resumeDistribution(cmaes_t *t, char *filename)
{
	int i, j, res, n;
	double d;
	FILE *fp = fopen(filename, "r");
	if (fp == NULL) {
		ERRORMESSAGE("cmaes_resumeDistribution(): could not open '",
			filename, "'", 0);
		return;
	}
	...
	while (i < n) {
		if ((res = fscanf(fp, " resume %lg", &d)) == EOF)
			FATAL("cmaes_resumeDistribution(): Unexpected error, bug", 0, 0, 0);
		else if (res == 0)
			fscanf(fp, " %*s");
		else if (res > 0)
			++i;
	...
	if (d != t->sp.N)
		FATAL("cmaes_resumeDistribution(): Dimension numbers do not match", 0, 0, 0);

	/* find next "xmean" entry */
	while (1) {
		if ((res = fscanf(fp, " xmean %lg", &d)) == EOF)
			FATAL("cmaes_resumeDistribution(): 'xmean' not found", 0, 0, 0);
		else if (res == 0)
			fscanf(fp, " %*s");
		else if (res > 0)
			break;
	...
	if (res != t->sp.N)
		FATAL("cmaes_resumeDistribution(): xmean: dimensions differ", 0, 0, 0);

	/* find next "path for sigma" entry */
	while (1) {
		if ((res = fscanf(fp, " path for sigma %lg", &d)) == EOF)
			FATAL("cmaes_resumeDistribution(): 'path for sigma' not found", 0, 0, 0);
		else if (res == 0)
			fscanf(fp, " %*s");
		else if (res > 0)
			break;
	...
	if (res != t->sp.N)
		FATAL("cmaes_resumeDistribution(): ps: dimensions differ", 0, 0, 0);

	/* find next "path for C" entry */
	while (1) {
		if ((res = fscanf(fp, " path for C %lg", &d)) == EOF)
			FATAL("cmaes_resumeDistribution(): 'path for C' not found", 0, 0, 0);
		else if (res == 0)
			fscanf(fp, " %*s");
		else if (res > 0)
			break;
	...
	if (res != t->sp.N)
		FATAL("cmaes_resumeDistribution(): pc: dimensions differ", 0, 0, 0);

	/* find next "sigma" entry */
	while (1) {
		if ((res = fscanf(fp, " sigma %lg", &d)) == EOF)
			FATAL("cmaes_resumeDistribution(): 'sigma' not found", 0, 0, 0);
		else if (res == 0)
			fscanf(fp, " %*s");
		else if (res > 0)
			break;
	...
	while (1) {
		if ((res = fscanf(fp, " covariance matrix %lg", &d)) == EOF)
			FATAL("cmaes_resumeDistribution(): 'covariance matrix' not found", 0, 0, 0);
		else if (res == 0)
			fscanf(fp, " %*s");
		else if (res > 0)
			break;
	...
	if (res != (t->sp.N * t->sp.N + t->sp.N) / 2)
		FATAL("cmaes_resumeDistribution(): C: dimensions differ", 0, 0, 0);
...
/* --------------------------------------------------------- */
double *const *
cmaes_SamplePopulation(cmaes_t *t, int id_rnd)
{
	...
	{ /* generate scaled cmaes_random vector (D * z)    */
		for (i = 0; i < N; ++i) {
			using namespace ofec;
			if (flgdiag)
				t->rgrgx[iNk][i] = xmean[i] + t->sigma * t->rgD[i] * GET_RND(id_rnd).normal.next();// cmaes_random_Gauss(&t->rand);
			else
				t->rgdTmp[i] = t->rgD[i] * GET_RND(id_rnd).normal.next();// cmaes_random_Gauss(&t->rand);
		}
		if (!flgdiag) {
...
/* --------------------------------------------------------- */
double const *
cmaes_ReSampleSingle_old(cmaes_t *t, double *rgx, int id_rnd)
{
	int i, j, N = t->sp.N;
	double sum;
...
/* --------------------------------------------------------- */
double *const *
cmaes_ReSampleSingle(cmaes_t *t, int iindex, int id_rnd)
{
	int i, j, N = t->sp.N;
	double *rgx;
	double sum;
...
/* --------------------------------------------------------- */
double *
cmaes_SampleSingleInto(cmaes_t *t, double *rgx, int id_rnd)
{
	int i, j, N = t->sp.N;
	double sum;
...
/* --------------------------------------------------------- */
double *
cmaes_PerturbSolutionInto(cmaes_t *t, double *rgx, double const *xmean, double eps, int id_rnd)
{
	int i, j, N = t->sp.N;
	double sum;
...
/* --------------------------------------------------------- */
const double *
cmaes_Optimize(cmaes_t *evo, double(*pFun)(double const *, int dim), long iterations, int id_rnd)
/* TODO: make signals.par another argument or, even better, part of cmaes_t */
{
		...
		(evo->gen < startiter + iterations || !iterations))
	{
		...
		pop = cmaes_SamplePopulation(evo, id_rnd); /* do not change content of pop */
	...
	char const *s3, char const *s4)
{
	...
	*/
	time_t t = time(NULL);
	FILE *fp = fopen((ofec::g_working_dir + "/result/errcmaes.err").c_str(), "a");
	if (!fp)
	{
		printf("\nFATAL ERROR: %s\n", s2 ? szCat(s1, s2, s3, s4) : s1);
