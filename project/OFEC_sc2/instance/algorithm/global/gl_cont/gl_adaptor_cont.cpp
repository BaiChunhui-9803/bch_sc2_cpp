...
namespace ofec {
	...
	void AdaptorContGL::updateProbability(int id_pro,
		...
		const std::vector<int>* index) 
	{
		...
		for (size_t j = 0; j < m_limit.size(); ++j) {
			m_limit[j].boundary.first = GET_CONOP(id_pro).range(j).second;
			m_limit[j].boundary.second = GET_CONOP(id_pro).range(j).first;
		}
		if (index == nullptr) {
	...
	void AdaptorContGL::createSolution(int id_pro, int id_rnd,
		std::vector<std::unique_ptr<Individual<>>>& pop, 
		std::vector<Individual<>>& offspring)
	{
		Real p;
		for (int i = 0; i < pop.size(); i++) {
			p = GET_RND(id_rnd).uniform.next();
			if (p <= m_alpha)
				localSearch(id_pro, id_rnd, i, pop, offspring);
			else
				globalSearch(id_pro, id_rnd, i, pop, offspring);
		}
	}

	void AdaptorContGL::localSearch(int id_pro, int id_rnd, size_t i, 
		std::vector<std::unique_ptr<Individual<>>>& pop, 
		std::vector<Individual<>>& offspring) 
	{
		Real center = 0, var = 0, x;
		const auto& range = GET_CONOP(id_pro).domain();
		for (int j = 0; j < offspring[i].variable().size(); j++) {
			...
			x = center + var * GET_RND(id_rnd).cauchy.next();
	...
	void AdaptorContGL::globalSearch(int id_pro, int id_rnd, size_t i, 
		std::vector<std::unique_ptr<Individual<>>>& pop, 
		std::vector<Individual<>>& offspring) 
	{
		Real center, var, x, p;
		...
		for (int j = 0; j < offspring[i].variable().size(); j++) {
			...
			p = GET_RND(id_rnd).uniform.next() * acc_pro[m_num - 1];
	...
	int AdaptorContGL::updateSolution(int id_pro, int id_alg,
		std::vector<std::unique_ptr<Individual<>>>& pop, 
		std::vector<Individual<>>& offspring, int& num_improve) {
		int rf = kNormalEval;
		int size_pop = pop.size();
		num_improve = 0;
		for (int i = 0; i < size_pop; i++) {
			if (!pop[i]->isActive()) continue;
			rf = offspring[i].evaluate(id_pro, id_alg);
			if (offspring[i].dominate(*pop[i], id_pro)) {
				*pop[i] = offspring[i];
				pop[i]->setImproved(true);
				num_improve++;
			}
			else {
				pop[i]->setImproved(false);
			}
	...
	void AdaptorContGL::updateStep(int id_pro, std::vector<std::unique_ptr<Individual<>>>& pop) {
		int popsize = pop.size();
		...
		for (int d = 0; d < numDim; ++d) {
			...
			Real lb = GET_CONOP(id_pro).range(d).first, ub = GET_CONOP(id_pro).range(d).second;
