...
namespace ofec {
	...
	public:
		...
		void addDistance(int id_pro,const InterpreterType& interpreter, 
			const std::vector<IndividualType*>& pop, 
			...
			//	std::cout << "popId\t" <<  popId << std::endl;
				interpreter.updateEdges(id_pro, *pop[popId]);
			...
			for (auto& popIter : pop) {
				m_curEdgeAvgDis += edgeDistance(id_pro, interpreter, *popIter);
			}
			m_curEdgeAvgDis /= pop.size();
	        	
			m_center = *pop[centerIdx];
			m_curEdgeBestDis = edgeDistance(id_pro, interpreter, m_center);

			m_curPosAvgDis = 0;
			for (auto& popIter : pop) {
				m_curPosAvgDis += posDistance(id_pro, *popIter);
			}
			m_curPosAvgDis /= pop.size();

			m_curPosBestDis = posDistance(id_pro, m_center);
		}
		Real edgeDistance(int id_pro, const InterpreterType& interpreter, IndividualType& cur) {
			interpreter.updateEdges(id_pro, cur);
			Real weight(0);
		...
		Real posDistance(int id_pro,IndividualType& cur) {
			return m_center.variableDistance(cur, id_pro);
		}
