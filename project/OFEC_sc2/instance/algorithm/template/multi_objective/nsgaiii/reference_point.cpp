...
namespace ofec {
	...
	int RefPoint::randomMember(int id_rnd) const {
		if (mv_potential_members_.size() > 0)
			return mv_potential_members_[GET_RND(id_rnd).uniform.nextNonStd(0, (int)mv_potential_members_.size())].first;
		else
			return -1;
	}
		...
		void generateRecursive(std::vector<RefPoint>* rps, RefPoint* pt, size_t num_objs, size_t left, size_t total, size_t element) {
			if (element == num_objs - 1) {
				...
				for (size_t i = 0; i <= left; i += 1) {
					...
					generateRecursive(rps, pt, num_objs, left - i, total, element + 1);
				}
			}
		}
