    ...
    void MQSOParticle::nextVelocity(const Solution<>* lbest, Real w, Real c1, Real c2, int id_rnd) {
        for (size_t j = 0; j < variable().size(); j++) {
            if (m_type == ParticleType::PARTICLE_NEUTRAL) {
                m_vel[j] = w * m_vel[j] + c1 * GET_RND(id_rnd).uniform.next() * (m_pbest.variable()[j] - variable()[j]) + c2 * GET_RND(id_rnd).uniform.next() * (lbest->variable()[j] - variable()[j]);
            }
            else if (m_type == ParticleType::PARTICLE_CHARGED) {
                m_vel[j] = w * m_vel[j] + c1 * GET_RND(id_rnd).uniform.next() * (m_pbest.variable()[j] - variable()[j]) + c2 * GET_RND(id_rnd).uniform.next() * (lbest->variable()[j] - variable()[j]) + mv_repulse[j];
            }
