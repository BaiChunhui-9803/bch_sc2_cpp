...
namespace ofec {
	...
	protected:
		PopACO<T_ACO> m_pops;
	protected:
	void updateFitness(int id_pro, IndividualType& sol) const;
	public:
	...
	inline void ACO<T_ACO>::updateBuffer() {
		...
		}
		{
			...
			for (int idx(0); idx < m_pops.size(); ++idx) {
				m_pops.getProInterpreter().updateEdges(m_id_pro, m_pops[idx], true);
			}
	...
	inline void ACO<T_ACO>::initialize_() {
		...
		m_pops.initializeParameters(m_id_param,m_id_rnd,m_id_pro,m_id_alg);
	...
	inline void ACO<T_ACO>::run_() {
		m_pops.initialize(m_id_rnd,m_id_pro, m_id_alg);
#ifdef OFEC_DEMO
		updateBuffer();
#endif
		while (!terminating()) {
			m_pops.evolve(m_id_rnd, m_id_pro, m_id_alg);
#ifdef OFEC_DEMO
			updateBuffer();
#endif
