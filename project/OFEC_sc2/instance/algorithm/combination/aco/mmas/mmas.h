...
namespace ofec {
	...
	protected:
		...
		std::vector<std::unique_ptr<IndividualType>> m_globalBest;//the globally best tour from the beginning of the trial
		std::vector<std::unique_ptr<IndividualType>> m_restartBest;//the restart best tour from the beginning of the trial
	...
	public:
		...
		bool stepNext(int id_rnd, int id_pro, int id_alg, const TInterpreter &interpreter, IndividualType &cur) override {
			...
			std::function<Real(int to)> nei_weight_fun = std::bind(&MMAS::getPro, this, std::cref(interpreter), id_pro, std::cref(cur), std::placeholders::_1);
			bool finish(false);
			std::vector<int> feasible(mvv_neighbors[interpreter.curPositionIdx(id_pro, cur)]);
			interpreter.fillterFeasible(id_pro, cur, feasible);
			if (!feasible.empty()) {
				auto next_step_iter = (GET_RND(id_rnd).uniform.greedyRandom(feasible.begin(), feasible.end(), nei_weight_fun));
				finish = interpreter.stepNext(id_pro, cur, *next_step_iter);
			}
			if (!finish) {
	...
	template<typename TInterpreter>
	inline void MMAS<TInterpreter>::updatePheromonePolicy(const std::vector<std::unique_ptr<IndividualType>> &update_sols) {
	...
	/*
	* 		bool updateSolutions(const IndividualType& cur, std::vector<std::unique_ptr<IndividualType>>& pops, std::vector<std::unique_ptr<IndividualType>>& curPops);

	*/

	template<typename TInterpreter>
	inline bool MMAS<TInterpreter>::updateSolutions(int id_pro, IndividualType &cur_indiv, std::vector<std::unique_ptr<IndividualType>> &curPops) {
